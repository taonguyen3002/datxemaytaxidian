...
const Editor = dynamic(() => import("@/components/EdittorWYSIWYG/Ckeditor/CKEditorComponent"));

const EditPost: React.FC = () => {
  const { breadcrumbs, setBreadcrumbs, addBreadcrumb, removeBreadcrumb, updateBreadcrumb } = useBreadcrumbs();
  //context use
  const { user } = useUser();
  //state
  ...
  useEffect(() => {
    ...
    async function fetchBlogData() {
      ...
      if (response && response.success) {
        ...
        setImage(blog.image ? { url: blog.image.url, alt: blog.image.alt ?? "" } : { url: "", alt: "" });
  ...
  return (
    <>
      {isLoad && <Loader />}
      <ClientMetadata title={title ? `Edit ${title}` : "Editor Blogs"} isIndex={false} />
      <div className="px-4 bg-transparent min-h-screen">
        ...
        >
          ...
          <TextField
            ...
            style={{ margin: "20px 0" }}
          />
          <TextField label="Tác giả" value={authorName} onChange={(e) => setAuthorName(e.target.value)} fullWidth />
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {urlImage && (
              ...
              >
                <img src={urlImage} alt={title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              </Box>
            )}
